...
'''
    ...
    1) ens1d
            1D NumPy array containing an ensemble of values for a forecast model variable
                Important note: this ensemble must be preprocessed to remove all duplicate 
                values and out-of-bounds values
    ...
    4) ens_moment1
            Mean of the ensemble. This is computed outside of the function because users
            may want to use the raw ensemble mean instead of the preprocessed ensemble
            mean

    5) ens_moment2
            Second raw moment of the ensemble. This is computed outside of the function 
            because users may want to use the raw ensemble's moment instead of the 
            preprocessed ensemble's moment.
...
'''
def BBRH_fit_dist_to_ens( ens1d, ens_moment1, ens_moment2, min_bound, max_bound ):
...
# GOING TO PUT THIS TESTOR OUTSIDE OF THE LIBRARY
